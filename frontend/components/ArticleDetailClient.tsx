  ...
  useEffect(() => {
    const loadArticle = async () => {
      try {
        ...
        setArticle(normalizeArticle(data as ArticleDetail & { id: string; title: string }))
        ...
        try {
          ...
          setArticle(normalizeArticle(fallbackData as ArticleDetail & { id: string; title: string }))
      ...
      {hasValidArticleUrl ? (
        <>
          ...
          <p className="mt-2 text-xs text-gray-500">
            출처 도메인: {sourceDomain || article.source || '확인 불가'}
          </p>
        </>
      ) : (
